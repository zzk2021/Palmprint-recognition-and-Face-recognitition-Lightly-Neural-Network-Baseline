...
class DefaultConfig:
    ...
    def __init__(self):
        ...
        # data
        self.DATA_DIR = "F:\行人重识别-论文\行人重识别_代码\data\StudentV233"  # dataset path
        self.DATALOADER_NUM_WORKERS = 8  # number of dataloader workers
        self.SAMPLER = 'triplet'  # batch sampler, option: 'triplet','softmax'
        self.BATCH_SIZE = 32  # MxN, M: number of persons, N: number of images of per person
        self.NUM_IMG_PER_ID = 4  # N, number of images of per person

        # model
        self.INPUT_SIZE = [256, 128]  # HxW
        self.MODEL_NAME = "ghostnetv2"  # backbone name, option: 'resnet50',
        self.LAST_STRIDE = 1  # the stride of the last layer of resnet50
        self.PRETRAIN_CHOICE = 'imagenet'
        self.PRETRAIN_PATH = "F:\行人重识别-论文\person-reid-tiny-baseline-master\pretrained_model\ck_ghostnetv2_10.pth.tar"  # pretrained weight path

        # loss
        self.LOSS_TYPE = 'triplet+softmax'  # option: 'triplet+softmax','softmax+center','triplet+softmax+center'
