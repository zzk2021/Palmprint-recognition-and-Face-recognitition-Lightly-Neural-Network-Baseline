...
class GhostBottleneckV2(nn.Module):
    ...
    def __init__(self, in_chs, mid_chs, out_chs, dw_kernel_size=3,
        ...
        if self.stride > 1 or layer_id==11:
    ...
    def forward(self, x):
        residual = x
        x = self.ghost1(x)
        if self.stride > 1 or self.layer_id==11:
            x = self.conv_dw(x)
            x = self.bn_dw(x)
...
class GhostNetV2(nn.Module):
    def __init__(self, cfgs, num_classes=1000, width=1.0, dropout=0.0, block=GhostBottleneckV2, args=None):
    ...
    def load_param(self, model_path):
        ...
        for i in param_dict:
            if 'classifier' in i or "total_ops" in i or "total_params" in i:
                continue
